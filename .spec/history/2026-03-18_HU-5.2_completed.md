...
- [x] Abrir edición de producto existente y eliminar una imagen; cerrar con `Cancelar` y confirmar que la imagen sigue visible tras recargar.
- [x] Eliminar una imagen y hacer cambios en texto; presionar `Guardar cambios` y confirmar borrado físico/persistencia final.
- [x] Repetir flujo con varias imágenes (incluyendo portada) y validar consistencia de orden y portada final.
- [x] Simular falla de commit (network/storage) y confirmar feedback recuperable + ausencia de pérdida silenciosa.
- [x] Verificar que acciones fuera de HU-5.2 (activar/desactivar producto, borrar producto desde tabla) no cambian su comportamiento actual.
...
## Completion
- **Date:** 2026-03-18
- **Estimated Duration:** ~295 min (sum of planned tasks)
- **Actual Duration:** ~0 min by commit-window metric (`git log --grep="feat(HU-5.2)"`, single commit at `22:06:20 -0600`)
- **Variance:** Commit-window duration is much shorter than estimate due to grouped implementation into one feature commit after iterative test/adjust cycles.
- **Files modified:**
  - `.spec/history/2026-03-18_HU-5.2_validation.md`
  - `app/src/components/admin/ImageGalleryManager.tsx`
  - `app/src/components/admin/ProductManager.tsx`
  - `app/src/test/admin-image-gallery-manager.test.tsx`
  - `app/src/test/product-manager-edit-gallery-deferred-delete.test.tsx`
  - `current_objective.md`
- **Tests added:** 3 (`app/src/test/product-manager-edit-gallery-deferred-delete.test.tsx`) plus regression adjustments in `admin-image-gallery-manager`.

## Deviations & Decisions
- **Added:** Dedicated integration suite for deferred deletion behavior; validation evidence file tracked in history.
- **Changed:** Verification focus shifted from optional DAL test changes to stronger UI+integration coverage in gallery/product edit flows.
- **Skipped:** No infra or DB schema updates were needed; `admin-catalog-service.ts` did not require additional code changes.
- **Key Decisions:** Physical deletion now executes only on product save; pending-deletion summary + undo added; reorder disabled while pending deletions exist to reduce state inconsistency.
- **Escalated ADRs:** None.
- **Lesson:** For destructive actions in admin UX, model them as staged intents and commit explicitly on final confirmation.
